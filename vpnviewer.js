...
module.exports.vpnviewer = function (parent) {
  ...
  // Общее хранилище ожидателей ответов агента
  ...
  // --- UI во вкладке устройства ---
  obj.exports = [ "onDeviceRefreshEnd" ];
  obj.onDeviceRefreshEnd = function () {
    if (pluginHandler && typeof pluginHandler.registerPluginTab === 'function') {
      pluginHandler.registerPluginTab({ tabTitle: "Плагины", tabId: "pluginVpnViewer" });
    }
    let nodeId = ""; try { nodeId = (window.currentNode && currentNode._id) || (window.node && node._id) || ""; } catch{}
  ...
  // серверная консоль (чтоб `plugin vpnviewer ping` на сервере не падал)

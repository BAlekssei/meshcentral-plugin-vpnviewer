// vpnviewer.js — гарантированный UI: вкладка (если сервер её создаёт)
// и fallback-кнопка в панели действий, плюс чтение файла по кнопке.

module.exports.vpnviewer = function (parent) {
  const obj = {};
  obj.parent = parent;

  // Функции, которые Web-UI будет дергать
  obj.exports = ['registerPluginTab', 'onWebUIStartupEnd', 'onDeviceRefreshEnd', 'goPageEnd'];

  // Пробуем зарегистрировать вкладку (если сервер собирает вкладки — появится p_vpnviewer)
  ...
  obj.goPageEnd = function (pageId) {
    // на всякий случай логируем смену страниц
  ...
  obj.onDeviceRefreshEnd = function (divid, currentNode) {
    try {
      ...
      // 2) иначе — добавляем кнопку в панель действий и свою панель ниже
      addFallbackButton();
  ...
  function addFallbackButton() {
    ...
    // пытаемся найти «полосу» с кнопками действий под заголовком узла
    const containers = [
    ...
    const bar = containers.find(Boolean);
    if (!bar) return;
    ...
    bar.appendChild(btn);
    console.log('[vpnviewer] fallback button added');
  }
  ...
  function ensureFallbackPanel(divid) {
    ...
    // куда вставить: основной контейнер страницы узла
    const host =

/**
 * VPN Viewer – вкладка «Плагины» и API read/write для агента
 */
"use strict";
...
module.exports.vpnviewer = function (parent) {
  ...
  function waitReplySafe(id, ms=15000) {
    return new Promise((resolve)=> {
      const t = setTimeout(()=>{ delete obj.pending[id]; resolve({ ok:false, error:'timeout' }); }, ms);
      obj.pending[id] = (payload)=>{ clearTimeout(t); delete obj.pending[id]; resolve(payload||{ok:true}); };
    });
  }
  ...
  // --- серверная консольная команда (чтобы `plugin vpnviewer ping` на СЕРВЕРЕ не падал) ---
  ...
  obj.handleAdminReq = async function (req, res, user) {
    ...
    if (action === "probe") {
      ...
      const r = await waitReplySafe(reqid, 5000);
  ...
  obj.serveraction = function (command) {
    try {
      ...
      switch (command.pluginaction) {
        ...
        default:             fn({ ok:false, error:'unknown agent reply: '+command.pluginaction }); break;
      }
    } catch (e) { /* ignore */ }
  };

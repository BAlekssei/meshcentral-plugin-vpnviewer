...
module.exports.vpnviewer = function (parent) {
  ...
  // Общий кэш «ожидателей» для ответов агента
  ...
  // В БРАУЗЕР экспортируем только клиентский хук!
  obj.exports = [ "onDeviceRefreshEnd" ];

  // Вкладка "Плагины" на карточке узла
  obj.onDeviceRefreshEnd = function () {
    ...
    } catch (e) { /* не роняем UI */ }
  };

  // Консоль сервера (не трогает агента)
  obj.consoleaction = function (args) {
    ...
    return (String(args[0]||'').toLowerCase()==='ping') ? 'vpnviewer server plugin: pong' : 'vpnviewer server plugin: console ok';
  };

  // HTTP: страница + API (этот метод НЕ нужно добавлять в exports)
  obj.handleAdminReq = async function (req, res, user) {
    ...
    if (action === "probe") {
      const agent = findAgent(node); if (!agent) { res.json({ ok:false, error:"Agent offline" }); return; }
      const reqid = rid();
      ...
      return res.json(await waitReply(reqid, 5000));
    }
    ...
    if (action === "read") {
      ...
      if (r.pluginaction === 'fileContent')    return res.json({ ok: !r.error, content: r.content || "", error: r.error || null });
      if (r.pluginaction === 'readFileResult') return res.json({ ok: !r.error, content: r.data    || "", error: r.error || null });
      return res.json(r);
    }
    ...
    if (action === "write" && req.method === "POST") {
      ...
      req.on("end", async () => {
        ...
        if (r.pluginaction === 'writeResult')     return res.json({ ok: r.ok === true, error: r.error || null });
        if (r.pluginaction === 'writeFileResult') return res.json({ ok: !r.error,      error: r.error || null });
        return res.json(r);
      });
      return;
    }
...
(function(){
  ...
  document.getElementById('bProbe').onclick = async ()=>{ S('Проверяю модуль…'); try{ const r=await api('probe'); S(r.ok?'OK':'timeout', r.ok);}catch(e){ S('Ошибка: '+e, false);} };
  ...
  // Приём ответов от агента (НЕ экспортируем)

// MeshAgent plugin: vpnviewer — РЕГИСТРАЦИЯ В pluginHandler.plugins.vpnviewer
(function () {
  // если по какой-то причине нет pluginHandler — выходим
  try { if (typeof pluginHandler !== 'object') { return; } } catch (e) { return; }
  ...
  // >>> РЕГИСТРАЦИЯ — ВАЖНО <<<
  try {
    // основной реестр, откуда команда `plugin <name>` дергает consoleaction
    pluginHandler.plugins = (typeof pluginHandler.plugins === 'object') ? pluginHandler.plugins : {};
    pluginHandler.plugins.vpnviewer = mod;

    // алиас на всякий случай (некоторые билды ещё смотрят сюда)
    pluginHandler.vpnviewer = mod;
  } catch (_) {}
})();
